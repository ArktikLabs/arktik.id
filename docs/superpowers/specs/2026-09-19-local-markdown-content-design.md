...
public/assets/blog/<filename>         the downloaded Contentful assets
```
...
image: string            optional, path under /public, e.g. /assets/blog/hero.jpg
...
- `public/assets/blog/` with the 26 assets.
...
2. Download every asset to `public/assets/blog/<original filename>`. On a filename
...
3. Convert each rich-text document to Markdown: headings to `#` levels,
   ...
   `![alt](/assets/blog/<filename>)`, bold and italic marks to `**` and `_`.
...
The space's default locale is `en-US` and every entry has English text, so
the script runs the entry pass twice: locale `id` writes `<slug>.id.md` and
locale `en-US` writes `<slug>.en.md`. Filenames use the `id` slug in both
passes, keyed by entry id. Authors are written once from `id`.
...
- `/en/` blog pages render the English content, and a document with no
  `.en.md` (a test fixture, or any future post) falls back to Indonesian.
...
- New English translations beyond what the Contentful `en-US` locale already holds.
